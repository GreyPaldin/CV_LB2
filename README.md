...
&ensp; Теперь создаётся матрица размерами N на M (где N и M - число блобов с первого и второго изображения), где индексы - номера блобов в каждом из изображений, а значения ячейки матрицы - степень сходства блобов. 

&ensp; Сходство блобов вычисляется на основе суммы 5 параметров блобов, описанных выше. Однако, это даст лишь степень различия блобов, поэтому результат сравнения будет вычитаться из еденицы, а после направляться в матрицу для обработки Венгреским алгоритмом (предыдущий абзац).

&ensp; После блобы будут отсортированы согласно парам и все лишние блобы будут удалены.
